...
data_labels = dict()
get_labels("train_label.csv")

total_documents = int(get_last_row("train_data.csv")[0])  # getting the total number of documents          
vocabulary_count = get_vocabulary_count("vocabulary.txt")

get_documents_in_each_category("train_label.csv")
...
get_words_in_each_class("train_data.csv")
for i in range(1,21):
...
##############################################
get_b_e("train_data.csv",vocabulary_count)
correct = 0
for i in range(1,21):
...
def test_get_documents_in_each_category(trainlabel_file):
    with open(trainlabel_file,'r') as f:
        row = None
        for row in csv.reader(f):
            i = int(row[0])
...
test_confusion_matrix = [ [0] * 21 for _ in range(21)]
test_mle_confusion_matrix = [ [0] * 21 for _ in range(21)]
test_total_documents = int(get_last_row("test_data.csv")[0])          
         
test_get_documents_in_each_category("test_label.csv")
test_data_labels=dict()
test_get_labels("test_label.csv")
...
test_get_b_e("test_data.csv",vocabulary_count)
